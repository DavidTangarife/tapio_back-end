...
const getGmailApi = (refresh_token, projectId, date) => __awaiter(void 0, void 0, void 0, function* () {
    //===============================
    // Setup Google Auth
    //===============================
    const auth_client = get_google_auth_client();
    auth_client.setCredentials({ refresh_token: refresh_token });
    const { token } = yield auth_client.getAccessToken();
    //===========================================
    ...
    const emails = yield gmail.users.messages.list({ userId: 'me', maxResults: 1000, q: query });
    console.log(emails);
    ...
    while (payload.length !== 0) {
        ...
        const cycle = new Promise((resolve) => setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            //=======================================
            const response = yield batchGetEmails(slice, token);
